...
def pull_process_and_push_data(device, device_attendance_logs=None):
    ...
    for device_attendance_log in device_attendance_logs[index_of_last+1:]:
        get_user_id = device_attendance_log['user_id']
        Scriptpath = os.path.abspath(os.path.dirname(__file__))
        current_date = datetime.datetime.today().strftime('%Y_%m_%d')
        PathSlash = "\\"
        user_data_folder =  Scriptpath + PathSlash + "user_data"
        user_file = user_data_folder + PathSlash + current_date + ".csv"
        last_user_file = os.listdir(user_data_folder)[0]

        if current_date + ".csv" != last_user_file:
            shutil.rmtree(user_data_folder)
            os.mkdir(user_data_folder)
            with open(user_file,'w') as user_data_f:
                user_data_f.write("user_id,status")
            user_data_f.close()    

        with open(user_data_folder+PathSlash+last_user_file,mode='r') as inp:
            reader = csv.reader(inp)
            dic_from_csv = {rows[0]:rows[1] for rows in reader}
        try:
            user_status = dic_from_csv[get_user_id]
            if user_status == "IN":
                dic_from_csv.update({get_user_id:"OUT"}) 
                status12 = "OUT"
            else:
                dic_from_csv.update({get_user_id:"IN"}) 
                status12 = "IN"  
        except:
            dic_from_csv[get_user_id] ="IN"
            status12 = "IN"
            inp.close()

        with open(user_data_folder+PathSlash+last_user_file,'w') as updata_file:
            for key1,value1 in dic_from_csv.items():
                updata_file.write("%s,%s\n" % (key1, value1))  
        updata_file.close()          
        punch_direction = device['punch_direction']
        if punch_direction == 'AUTO':
            #################### uper change ###############
            if device_attendance_log['punch'] in device_punch_values_OUT:
                punch_direction = status12
            elif device_attendance_log['punch'] in device_punch_values_IN:
                punch_direction = status12 # IN
            else:
        ...
        if erpnext_status_code == 200:
            ...
            attendance_failed_logger.error("\t".join([str(erpnext_status_code), str(device_attendance_log['uid']),
                ...
                str(device_attendance_log['punch']), str(device_attendance_log['status']),
                json.dumps(device_attendance_log, default=str)]))
            # if not(any(error in erpnext_message for error in allowlisted_errors)):   ################### comment by me #####################
